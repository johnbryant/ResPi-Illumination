...
//  ResPiIllumination
//
//  Created by JohnBryant on 6/15/16.
...
class VoiceControlViewController : UIViewController, AVAudioRecorderDelegate, IFlySpeechRecognizerDelegate {
    ...
    @IBAction func sendCommand(sender: AnyObject) {
        if !isRecording && self.voiceText.text != "" {
            let alert = UIAlertController(title: "已执行", message: "", preferredStyle: .Alert)
    ...
    func resolveVoiceResult(string: String) {
        var results = [String]()
        if string == "\"。\"" {
